...
describe('QState', function() {
  ...
  describe('#applyOperatorMatrix', function() {
    const matrix = [
    ...
    it('applies the matrix to a 0 bit', function() {
    ...
    it('applies the matrix to a 1 bit', function() {
  ...
  describe('#applyToOneBit', function() {

    it('does nothing when control bits are zero', function() {
      const result = QState.applyToOneBit([1, 2], 0, hadamardMatrix, Q('|001>'));
      expect(result.toString()).to.equal('|001>');
    });

    it('applies an operator to a qubit (when control bits always match)', function() {
    ...
    it('applies an operator to a qubit (when control bits match only for some states)', function() {
      // Initial state: (0.712)|101> - (0.712)|111>
      const initialAmplitudes = [];
      initialAmplitudes[5] = Complex.SQRT1_2;
      initialAmplitudes[7] = Complex.SQRT1_2.multiply(-1);
  ...
  describe('#controlledApplicationOfqBitOperator', function() {
    ...
    it('invokes the qbitFunction when the control bit specifier is null', function() {
      const x = Q('|000>').controlledApplicationOfqBitOperator(null, 0, hadamardMatrix);
      expect(x.toString()).to.equal('(0.7071)|000> + (0.7071)|001>');
    });

    it('invokes the qbitFunction on a range of target bits', function() {
      const targetBits = {from: 0, to: 1};
  ...
  describe('#each', function() {
    it('should invoke a callback with a QStateComponent', function() {
